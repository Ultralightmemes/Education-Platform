...
#     url = reverse('education-follow-course', kwargs={'pk': create_test_course.pk})
...
def test_category_list(create_test_category, api_client):
    url = reverse('education-category-list')
    response = api_client.get(url)
    assert response.status_code == 200
...
def test_category_detail(create_test_category, api_client):
    url = reverse('education-category-detail', kwargs={'pk': create_test_category.pk})
    response = api_client.get(url)
    assert response.status_code == 200
...
def test_course_lesson_list(api_client_with_credentials, create_test_lesson):
    url = reverse('education-lesson-list', kwargs={'course_pk': create_test_lesson.theme.course.pk})
    response = api_client_with_credentials.get(url)
    assert response.status_code == 200
...
def test_course_lesson_detail(api_client_with_credentials, create_test_lesson):
    url = reverse('education-course-lesson-detail', kwargs={'pk': create_test_lesson.pk,
                                                            'course_pk': create_test_lesson.theme.course.pk,
                                                            })
    response = api_client_with_credentials.get(url)
    assert response.status_code == 200
...
def test_user_lesson_answer(api_client_with_credentials, create_test_lesson):
    url = reverse('education-course-lesson-answer',
...
def test_user_courses(user_with_course_subscription):
    api_client, pk = user_with_course_subscription
    url = reverse('education-course-get-user-courses')
    response = api_client.get(url)
    assert response.status_code == 200
